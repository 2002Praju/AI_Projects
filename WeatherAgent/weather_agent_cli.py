...
def call_claude_sonnet(prompt):
    ...
    Args: 
        prompt (str): The question or instructions we want to send to Claude

    Returns:
        tuple: (success: bool, response: str) - success status and Claude's response as error message
    """
...
- ZIP codes: Estimates based on the area
...
Example for Seattle:
https://api.weather.gov/points/47.6.62,-122.3321

Example for largest city in USA:
Based on your knowledge, you will establish location in New York City
...
3. The next 2-3 days outlook with key details (temperature, percipitation, wind)
